...
- **`useOrderCalc` is now generic over the calc response and no longer depends on the `API` types.** `calcHandler` may resolve to any response carrying `from_amount` / `result_amount` (the v2 `Calc.Response`, the frontend `orders.frontend.calc` `OrderCalculation`, etc.), so a single hook covers every calc endpoint. `calcData` preserves the caller's full response plus the calculation flags — `TResponse & { is_reverse: boolean; is_subtract: boolean }` — so consumers keep reading rate/fee/currency fields off it. `UseOrderCalcProps` / `UseOrderCalcData` take the same `TResponse` type parameter. **Breaking (type-only):** there is no default type parameter — `useOrderCalc(...)` infers `TResponse` from the passed `calcHandler`, and bare references to `UseOrderCalcProps` / `UseOrderCalcData` now require an explicit type argument. Runtime behaviour is unchanged.
